...
def run():
    ...
    for producer in ('quotes','feedback'):
        for name in OUTPUTS[producer] | {producer+'_manifest.json'}:
            (site/name).unlink(missing_ok=True)
        if os.getenv('OUTCOME_'+producer.upper()) != 'success': continue
        try:
            validate(producer)
        except (ValueError, KeyError, TypeError, OSError):
            continue  # An expired quote or broken shadow cannot block the scan.
        for name in OUTPUTS[producer]:
            shutil.copy2(name,site/name)
            links.append((name,producer+' — production distincte, voir son manifeste'))
        shutil.copy2(producer+'_manifest.json',site/(producer+'_manifest.json'))
