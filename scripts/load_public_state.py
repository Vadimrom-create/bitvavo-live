...
JOURNALS=['history','history_corrected','history_legacy_diagnostics','comparison_history','publication_history','scan_attempts',
          'prospective_sessions','prospective_observations','feedback_history']
...
def load(source, target='.', owner='prospection'):
    ...
    if owner=='feedback': names=['feedback_state','feedback_history']
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--source',required=True);p.add_argument('--owner',choices=['prospection','evaluation','feedback'],required=True)
