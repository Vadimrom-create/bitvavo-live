...
## Lot 12 — pilote technique et préparation du gel (2026-09-15)

- Reprise sur `3cac99c51d482cb93fcada65b4357b126b229172`, branche `codex/astra-phase3-20260911` : les neuf fichiers partiels attendus étaient présents ; aucun travail des lots précédents n'a été reconstruit.
- Sessions prospectives versionnées ouvertes avant acquisition ; inscriptions immuables et idempotentes, censure et données absentes explicites, rapports à sources identifiées, contrastes commun/native et dispersion par blocs regroupant les marchés. Le code modifié sépare les nouvelles cohortes au lieu de réinterpréter les anciennes.
- Protocole volontairement non renseigné ; gel futur conditionné à des preuves prospectives, des justifications statistiques, une durée minimale de trente jours neufs, une purge et un embargo. Aucun gel réel, held-out, effectif scientifiquement validé ou droit de promotion produit dans cette session.
- Correction finale : la publication historique ne possède pas les rapports prospectifs. Leur génération et publication sont liées dans la même invocation ; archive et latest doivent correspondre au résultat courant. Un échec empêche aussi l'artefact prospectif de ce run. Deux tests supplémentaires couvrent le rapport ancien après erreur et la concordance avant publication.
- Validation : **15 tests prospectifs ciblés, 154 tests de non-régression réussis**. La première tentative complète signalait uniquement `requests` absent après reconnexion ; réinstallation des dépendances existantes de l'exécuteur, sans changement des fichiers de dépendances, puis suite complète réussie.
- Replay réel V4 `20260911T115709Z-f616cf19` : référence gelée = code instrumenté = résultat enregistré, 50 lignes de watch. **113/113 journaux DL-V1** rejoués à l'identique sur tous les champs retournés par `decide`. Les fixtures de pipeline legacy/corrigé, l'index reconstructible, la publication et l'isolation du monitoring passent dans la suite complète.
- **328 empreintes protégées inchangées** ; aucun historique, seuil, poids ou référence V4 modifié. L'archive `20260908T205101Z-34cfba07` est un gzip/JSON valide, identique aux octets de HEAD, et absente du diff. Contrôles de syntaxe Python/JSON, YAML des deux workflows, unicité des IDs d'étapes et `git diff --check` réussis.
- Documentation opérationnelle et rollback : `PHASE3_LOT12_PROSPECTIVE_PROTOCOL.md`. Les étapes facultatives peuvent être désactivées en conservant tous les journaux ; le monitoring critique ne dépend pas du pilote.
- Pas de push ni de déploiement dans cette session. Le pilote réel reste à démarrer après publication ; monitoring réel et alertes privées : **PENDING PRIVATE CONFIGURATION**. DL-V2 et support/reprise restent sans route active. La partie technique de l'étape 12 termine les implémentations du plan, qui ne définit aucune étape 13 ; validation future et promotion demeurent distinctes.
