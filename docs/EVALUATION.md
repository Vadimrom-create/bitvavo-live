...
## Lot 11 — évaluation hors du cycle et index reconstructible

`pipeline.py` ne reconstruit plus tout l'historique et n'appelle plus l'évaluateur exhaustif. Il publie le contrôle des listes courantes et un statut explicite `SEPARATE_EVALUATION_WORKFLOW` pour le contrôle historique, jamais une liste vide signifiant artificiellement « aucun événement ».

`scripts/run_evaluation.py` indexe chaque `data_policy` dans une base SQLite distincte, produit `evaluation_policies/`, `evaluation.json`, `comparison_report.json` et `market_control_history.json`, et conserve un rapport immuable dans `evaluation_history/`. Le manifeste lie les hashes des journaux, la révision de code et les empreintes des rapports. L'alias `evaluation.json` expose encore les métriques historiques, en plus de leur contexte versionné. Les fonctions et seuils de `research/evaluation.py` restent inchangés dans ce lot.

L'index vérifie l'identifiant et les empreintes sémantique/compressée de chaque source. Modification ou disparition d'une source : erreur explicite. Une transaction interrompue est rejouable. Une base dérivée corrompue est mise de côté puis reconstruite. Le choix de la première bougie est fixé par l'observation enregistrée `(scan_ts, scan_id)`, indépendamment de l'ordre d'ingestion. Les sources historiques ne sont jamais mises à jour par l'index.

L'état léger de prospection conserve la récurrence sur deux heures et les 200 dernières identités de bougies par marché. Il conserve les marchés temporairement absents. Un état perdu se reconstruit sur les journaux récents ; d'anciennes bougies peuvent alors réapparaître dans un delta, mais le premier enregistrement reste canonique dans l'index. Les profils V4 et leur état algorithmique ne sont pas remplacés par cet état descriptif.

Les reçus `publication_history/` sont créés après l'acquittement du push. Ils donnent les bornes de temps de publication ; la borne supérieure est utilisée prudemment par la comparaison native. Ils ne remplacent pas les horodatages initiaux des décisions et ne prouvent aucun email reçu. Une confirmation non persistée demeure inconnue dans le comparateur. Les tentatives en échec et les intervalles sans scan sont visibles séparément ; leurs événements de marché non observés restent inconnus. Les acquisitions/publications ne prouvent pas une supervision privée réussie.

Validation reproductible : `python scripts/verify_index_parity.py --output runtime/lot11_historical_parity.json`. Le 13 septembre, les 197 journaux locaux (84 522 observations, 152 128 bougies) donnent les mêmes tables et résultats d'évaluation que le code pré-lot-11, y compris après interruption/reprise et ingestion inverse. Il s'agit d'une preuve de reproductibilité, pas d'une validation statistique.
